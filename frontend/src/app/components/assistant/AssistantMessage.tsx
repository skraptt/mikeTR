                ...
                try {
                    ...
                    onError?.({
                        ...
                        versionId: annotation.version_id ?? null,
                            message:
                                verb === "accept"
                                    ? "Bir veya daha fazla onay kaydedilemedi."
                                    : "Bir veya daha fazla red kaydedilemedi.",
                    });
                }
                done++;
...
function EditCardsSection({
    ...
    const summary =
        pending.length > 0
                        ? docCount > 1
                                ? `${pending.length} izlenen değişiklik ${docCount} belgede`
                                : `${pending.length} izlenen ${pending.length === 1 ? "değişiklik" : "değişiklikler"}`
                        : docCount > 1
                            ? `${resolvedCount} çözümlenmiş izlenen değişiklik ${docCount} belgede`
                            : `${resolvedCount} çözümlenmiş izlenen ${resolvedCount === 1 ? "değişiklik" : "değişiklikler"}`;
        ...
        <div className="border border-gray-200 rounded-lg bg-white overflow-hidden">
            ...
            <div className="flex items-center gap-2 px-3 pt-3">
                ...
                <button
                    ...
                    aria-label={isOpen ? "Düzenlemeleri daralt" : "Düzenlemeleri genişlet"}
...
function DocFindBlock({
    ...
    const matchSuffix = isStreaming
        ? ""
        : ` (${totalMatches} eşleşme${totalMatches === 1 ? "" : "ler"})`;
    return (
        <div className="flex items-start text-sm font-serif text-gray-500 relative">
            {showConnector && (
                <div className="absolute bottom-0 w-[1px] bg-gray-300 top-[13px] left-[2.5px] h-[calc(100%+11px)]" />
            ...
            <div className="ml-2 min-w-0 flex-1 whitespace-normal break-words">
                ...
                <span>
                    ...
                    <span className="ml-1 text-gray-400">içinde {filename}</span>
                    {isStreaming && "..."}
                </span>
            </div>
        </div>
...
function DocReplicatedBlock({
    ...
    const label = isStreaming ? "Kopyalanıyor" : "Kopyalandı";
    const suffix =
        !isStreaming && count > 1 ? ` ${count} kez` : isStreaming ? "..." : "";
    return (
        <div className="flex items-start text-sm font-serif text-gray-500 relative">
            {showConnector && (
                <div className="absolute left-0 top-0 bottom-0 w-[1px] bg-gray-300 top-[13px] left-[2.5px] h-[calc(100%+11px)]" />
...
function DocDownloadBlock({
    ...
    const extMatch = filename.match(/\.(\w+)$/);
            const ext = extMatch ? extMatch[1].toUpperCase() : "DOSYA";
