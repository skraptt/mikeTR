    ...
    return (
        <div className="flex-1 overflow-y-auto bg-white flex flex-col h-full">
                ...
                <div>
                    <div className="flex items-center gap-1.5 text-2xl font-medium font-serif">
                        ...
                        {tab !== "documents" && (
                            <>
                                <span className="text-gray-300">›</span>
                                    <span className="text-gray-900">{tab === "assistant" ? "Asistan" : "Tablolu İncelemeler"}</span>
                            </>
                        )}
                    </div>
                </div>
                    ...
                    <div className="flex-1 flex flex-col min-h-0">
                        ...
                        {docs.length === 0 && folders.length === 0 ? (
                            ...
                            >
                                ...
                                <p className="text-sm text-gray-400">PDF veya DOCX dosyalarını buraya bırakın</p>
                            </div>
                        ) : (
                            <div
                ...
                {tab === "assistant" && (
                    <>
                        <div className="flex items-center h-8 pr-8 border-b border-gray-200 text-xs text-gray-500 font-medium select-none">
                            ...
                            <div className="ml-auto w-32 shrink-0 text-left">Oluşturulma</div>
                            <div className="w-8 shrink-0" />
                        </div>
                        {chats.length === 0 ? (
                            <div className="flex flex-col items-start py-24 w-full max-w-xs mx-auto">
                                ...
                                <p className="mt-1 text-xs text-gray-400 max-w-xs">Bu projedeki belgelere dayalı sorular sorun ve yanıtlar alın.</p>
                                            ...
                                            <RowActions
                                                onRename={() => {
                                                    if (user?.id && chat.user_id !== user.id) {
                                                        setOwnerOnlyAction("bu sohbeti yeniden adlandır");
                                                        return;
                                                    }
                                                ...
                                                onDelete={async () => {
                                                    if (user?.id && chat.user_id !== user.id) {
                                                        setOwnerOnlyAction("bu sohbeti sil");
                                                        return;
                                                    }
                ...
                {tab === "reviews" && (
                    <>
                        <div className="flex items-center h-8 pr-8 border-b border-gray-200 text-xs text-gray-500 font-medium select-none">
                            ...
                            <div className="ml-auto w-24 shrink-0 text-left">Sütunlar</div>
                            <div className="w-24 shrink-0 text-left">Belgeler</div>
                            <div className="w-32 shrink-0 text-left">Oluşturulma</div>
                            <div className="w-8 shrink-0" />
                        </div>
                        {projectReviews.length === 0 ? (
                            <div className="flex flex-col items-start py-24 w-full max-w-xs mx-auto">
                                ...
                                <p className="text-2xl font-medium font-serif text-gray-900">Tablolu İncelemeler</p>
                                <p className="mt-1 text-xs text-gray-400 max-w-xs">Yapay zekayı kullanarak proje belgelerinden tablo halinde veri çıkarın.</p>
                                            ...
                                            <RowActions
                                                onRename={() => {
                                                    if (user?.id && review.user_id !== user.id) {
                                                        setOwnerOnlyAction("bu tablolu incelemeyi yeniden adlandır");
                                                        return;
                                                    }
                                                ...
                                                onDelete={async () => {
                                                    if (user?.id && review.user_id !== user.id) {
                                                        setOwnerOnlyAction("bu tablolu incelemeyi sil");
                                                        return;
                                                    }
            ...
            <AddDocumentsModal
                ...
                breadcrumb={["Projeler", project.name + (project.cm_number ? ` (${project.cm_number})` : ""), "Belgeler ekle"]}
                projectId={projectId}
            />
